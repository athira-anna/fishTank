...
import { getImageUrl, restoreFishImage } from "@/lib/api";
import { parseDrawingData } from "@/lib/fishDrawingTypes";
import { renderFishFromDrawing } from "@/lib/renderFishFromDrawing";
import { computeFishZIndex, depthStyle } from "@/lib/tankScenery";
import type { Fish, SwimmingFish } from "@/types/fish";
...
interface FishSpriteProps {
  ...
  onImageRestored?: (fish: Fish) => void;
  isHovered: boolean;
}
...
export default function FishSprite({
  ...
  const remoteUrl = previewUrl ?? getImageUrl(fish.image_url);
  const displaySrc = previewUrl ?? fallbackSrc ?? remoteUrl;
  ...
  }, [previewUrl, remoteUrl, fish.image_url]);

  async function handleImageError() {
    if (restoreStarted.current || previewUrl || !fish.drawing_data) return;
    restoreStarted.current = true;

    const drawing = parseDrawingData(fish.drawing_data);
    if (!drawing) return;

    try {
      const blob = renderFishFromDrawing(drawing);
      const objectUrl = URL.createObjectURL(blob);
      fallbackUrlRef.current = objectUrl;
      setFallbackSrc(objectUrl);

      const updated = await restoreFishImage(fish.id, blob);
      onImageRestored?.(updated);
      ...
      /* keep local fallbackSrc so the fish still shows */
    }
  }
